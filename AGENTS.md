...
각 Agent는 아래 역할과 권한 경계를 엄격히 준수합니다.
...
- **Low** (문구/스타일/단순 UI/문서 수정)
  - 조건: CI 통과 + 사람 1명 승인
- **Medium** (일반 CRUD, 일반 API, 일반 버그 수정, 테스트 추가)
  - 조건: CI 통과 + QA 결과 확인 + 사람 1명 승인
- **High** (인증/인가, 외부 API 연동, DB Migration, 공통 API 계약 변경, 관리자 기능, CI/인프라 변경)
  - 조건: 사람 집중 검토 + Tech Lead 또는 코드 소유자 승인 (필요 시 2명) + 명시적 Rollback 계획
- **Critical** (결제/환불, 개인정보 처리, Secret 노출, 운영 배포, DB 삭제/파괴적 변경, 인증 토큰 정책 근본 변경)
  - 조건: 자동 진행 즉시 중지 + 사람 명시적 승인 + 책임자 검토 + 별도 보안/운영 검토 기록 필수
